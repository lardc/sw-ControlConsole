...
function CGTU_Probe(ProbeCMD)
{
	...
	if (ProbeCMD == 110)
	{
		...
		// Set error
		cgtu_vgt_set_err.push(((vgt_set - vgt_sc) / vgt_sc * 100).toFixed(2));
		// Summary error
	...
	if (ProbeCMD == 111)
	{
		...
		// Set error
		cgtu_igt_set_err.push(((igt_set - igt_sc) / igt_sc * 100).toFixed(2));
		// Summary error
		E0 = Math.sqrt(Math.pow(EUosc, 2) + Math.pow(ER, 2));
	...
	if (ProbeCMD == 112)
	{
		...
		// Set error
		cgtu_vd_set_err.push(((vd_set - vd_sc) / vd_sc * 100).toFixed(2));
		// Summary error
		E0 = Math.sqrt(Math.pow(EUosc, 2) + Math.pow(ER, 2));
	...
	if (ProbeCMD == 113)
	{
		...
		// Set error
		cgtu_id_set_err.push(((id_set - id_sc) / id_sc * 100).toFixed(2));
		// Summary error
		E0 = Math.sqrt(Math.pow(EUosc, 2) + Math.pow(ER, 2));
...
var Vg_FeedBack = [];
var Vg_Error = [];
function CGTU_TestRegulator_Vg(Vg_Value, Kp, Ki)
{	
	dev.w(131, 1000);
	dev.w(130,Vg_Value);
	dev.w(10, Kp);
	...
	Vg_Target = dev.rafs(9);
	Vg_FeedBack = dev.rafs(1);
	
	for(i=0;i<Vg_FeedBack.length;i++)
	{
		Vg_Error[i] = Vg_Target[i] - Vg_FeedBack[i];
	}
}

function CGTU_PlotError()
{
	pl(Vg_FeedBack);
	pl(Vg_Error);
	pl(dev.rafs(5));
...
function CGTU_RegulatorsDisable()
{	
	R1 = dev.r(10);
	R2 = dev.r(11);
	R3 = dev.r(14);
...
function CGTU_RegulatorsEnable()
{
	dev.w(10,R1);
	dev.w(11,R2);
	dev.w(14,R3);
